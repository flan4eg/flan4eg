...
def get_address():
	...
	all_data = []  # мне подсказали, что лучше открывать файл только один раз и для этого сделать новую переменную all_data
	with open('randominfo/data.csv', 'r', encoding='utf-8') as addr_file:
		reader = csv.reader(addr_file)
		for row in reader:
			all_data.append(row)
